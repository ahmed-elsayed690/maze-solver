...
void MazeSolverGui::editMaze() {
    int r, c;
    bool ok;
    char action = QInputDialog::getText(this, "Edit Maze", "Enter row and column to edit (0-based index) followed by action (0 for empty, 1 for obstacle):").toStdString()[0];
    if (action == '1') {
        maze[r][c] = WALL;
    } else {
        maze[r][c] = EMPTY;
